...
public class FasterVRBCTestNG extends StatefullVRBCTestNG {
	...
	public void testDelta() throws Exception{
    	...
    	FasterVRBC fvrbcAlg = new FasterVRBC(g_line, cw, ShadowedHistoriedCacheArr.CACHEID);
    	for (int i=0; i<g_line.getDeltaValues().length; i++){
    		int[] group = g_line.getDeltaGroups()[i][0];
    		double actual = fvrbcAlg.getDelta(Index.valueOf(group[0]), Index.valueOf(group[1]), Index.valueOf(group[2]));
    	...
    	fvrbcAlg = new FasterVRBC(g_middleV, cw, ShadowedHistoriedCacheArr.CACHEID);
    	...
    	fvrbcAlg = new FasterVRBC(g_crossedCircle, cw, ShadowedHistoriedCacheArr.CACHEID);
    ...
    public void testSourceDependency() throws Exception{
    	...
    	for (int i=0; i<g_line.getSourceDependencyValues().length; i++){
    		...
    		double actual = fvrbcAlg.getSourceDependency(Index.valueOf(group[0]), Index.valueOf(group[1]));
    ...
    public void testTargetDependency() throws Exception{
    	...
    	for (int i=0; i<g_line.getTargetDependencyValues().length; i++){
    		...
    		double actual = fvrbcAlg.getTargetDependency(Index.valueOf(group[0]), Index.valueOf(group[1]));
    ...
    public void testBetweenness() throws Exception{
    	...
    	for (int i=0; i<g_line.getBetweennessValues().length; i++){
    		...
    		double actual = fvrbcAlg.getBetweeness(Index.valueOf(v));
