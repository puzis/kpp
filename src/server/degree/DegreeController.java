...
public class DegreeController 
{
	...
	public int create(int netID){
		...
		if (graph != null){
			...
			try{
				...
				LoggingManager.getInstance().writeSystem("An exception has occured while creating DegreeAlgorithm:\n" + ex.getMessage() + "\n" + LoggingManager.composeStackTrace(ex), DegreeController.ALIAS, "create", ex);
			}
		}
		int algID = DataBase.putAlgorithm(degree, netID);
		return algID;
	}
	...
	public Object[] getDegree(int algID, int[] vertices){
		...
		for (int i = 0; i < vertices.length; i++)
			degreeValues[i] = dAlg.getDegree(vertices[i]);
		return degreeValues;
	}
	...
	public Object[] getDegree(int algID){
		...
		int i = 0;
		while (d.hasNext())
			dVals[i] = new Integer(d.next().intValue());
		return dVals;
	}
	...
	public Object[] getCentralEdges(int algID, int k, Object[] candidatesObj, Object[] givenVerticesObj, Object[] givenEdgesObj)
	{
		...
		try{
			AbstractSimpleEdge<Index,BasicVertexInfo>[] ce = AbsGreedyDegree.findEdges(degreeAlgorithm, Algorithm.TopK, candidates, givenVertices, givenEdges, Bound.GroupSize, k, new DummyProgress(), 1);
